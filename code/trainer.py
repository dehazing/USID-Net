...
import os
from loss_functions import DCLoss
...
class UNIT_Trainer(nn.Module):
    ...
    def gen_update(self, x_a, x_b, hyperparameters):
        ...
        # dark channel loss
        self.loss_dc = self.compute_dcloss(x_ab) if hyperparameters['dcloss_w'] > 0 else 0

        #identity mapping loss
        self.criterionIdt = torch.nn.L1Loss()

        h_a_idt = self.gen_a.encode_cont(x_b)
        x_bb_idt = self.gen_b.decode_cont(h_a_idt + noise_b)
        self.loss_idt = self.criterionIdt(x_bb_idt, x_b)

        self.loss_unsupervised = hyperparameters['dcloss_w'] * self.loss_dc + \
                                 hyperparameters['idtloss_w'] * self.loss_idt
        ...
        self.loss_gen_total = hyperparameters['gan_w'] * self.loss_gen_adv_a + \
                              ...
                              hyperparameters['gan_w'] * self.loss_unsupervised
    ...
    def compute_dcloss(self, img, patch=35):
        img = img.add(1).mul(0.5)
        dc_loss = DCLoss(img, patch)
        return dc_loss

    def sample_single_img(self, x_a, x_b):
        ...
        for i in range(x_a.size(0)):
            ...
            x_ab.append(self.gen_b.decode_cont(h_a))#enhance
    ...
    def sample(self, x_a, x_b):
        ...
        x_a_recon = self.gen_a.decode_recs(h_aa_cont)
    ...
    def dis_update(self, x_a, x_b, hyperparameters):
        ...
        x_ab = self.gen_b.decode_cont(h_a + noise_a)#enhance
        # D loss
